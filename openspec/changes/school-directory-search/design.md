...
- 不在這次匯入國中、高中名錄——先只做國小，涵蓋目前主要使用情境；國中/高中資料格式相同，之後要擴充只需要在匯入來源清單加一筆

## Decisions

### 資料來源與匯入方式

一次性匯入教育部統計處的國小名錄（dataset 6087，`https://stats.moe.gov.tw/files/school/114/e1_new.json`）到自己的 D1，寫成可重複執行的匯入 script（用 `INSERT OR REPLACE`，以 SchoolCode 為鍵），之後每學年跑一次即可。**先只做國小**（目前主要使用情境），國中（6088）、高中（6089）資料格式完全一致，之後要擴充只需要在匯入 script 的來源清單加一筆，不需要改動 schema 或搜尋/驗證邏輯。

**2026-09-05 更新**：實作時原本一次匯入三個教育階段（3855 校），使用者決定先縮小範圍只做國小（2608 校），國中/高中列為後續擴充。
