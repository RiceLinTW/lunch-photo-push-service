...
- [x] 4.1 依 前端選校流程改版 決策，實作縣市下拉 + 學校名稱關鍵字搜尋 UI，呼叫 `/api/schools/search` 顯示候選清單；驗證：輸入關鍵字後畫面顯示對應候選學校清單
- [x] 4.2 實作選定候選學校後呼叫 `/api/schools/resolve`：驗證通過才顯示訂閱按鈕並帶入確認後的 `SchoolId`；驗證失敗或搜尋不到時，落實 Manual entry remains available as a fallback，顯示連到官方查詢頁的備援文字與手動輸入代碼欄位；驗證：模擬驗證回傳「無法確認」，畫面正確顯示手動輸入備援而非卡住或報錯
